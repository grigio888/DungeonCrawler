...
/**
 * Expected folder for a monster sprite: `src/lib/monsters/{monsterId}/sprites/`.
 * @param {string} monsterId
 * @param {string} [fileName]
 */
export function getMonsterSpritePath(monsterId, fileName = `${DEFAULT_MONSTER_SPRITE}.png`) {
	return `src/lib/monsters/${monsterId}/sprites/${fileName}`;
}
...
export function listMonsterSpriteFiles(globEntries, monsterId) {
	const prefix = `/monsters/${monsterId}/sprites/`;
	const files = [];
